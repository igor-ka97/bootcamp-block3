...
<main class="inside-content">
	...
	<form class="search-filter" id="catalog-page__search-filter-1" method="GET" action="catalog.php">
		...
		<span class="search-filter__item">
			...
			<input class="search-filter__input" step="0.01" type="number" min="0" name="price_from" id="price-from" value="<?=(isset($_GET['price_from'])) ? $_GET['price_from'] : null?>" placeholder="от">
		</span>
		<span class="search-filter__item">
			<label class="search-filter__label" for="price-to">—</label>
			<input class="search-filter__input" type="number" min="0" name="price_to" id="price-to" value="<?=(isset($_GET['price_from'])) ? $_GET['price_to'] : null?>" placeholder="до">
		</span>
		...
		<?if($products):?>
			<?foreach($products as $product):?> 
				<li class="category good-piece">
					<a class="category__link" href="/product.php?id=<?=$product['product_id'].$category?>">
